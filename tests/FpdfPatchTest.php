 ...
 *  3. \Atgp\FacturX\Writer::generate() s'exécute dans le même process sans
 *     conflit de classe — le raison d'être du renommage.
 */
...
class FpdfPatchTest extends TestCase
{
    ...
    public function testWriterInjectsXmlIntoPdfInProcess(): void
    {
        // Génère un PDF minimal avec SetasignFPDF directement dans le process
        // (impossible avant le renommage à cause du conflit de classe avec FPDF).
        ...
        $this->assertStringStartsWith('%PDF', $pdfContent, 'SetasignFPDF doit produire un PDF valide');

        // Génère un XML Factur-X EN16931 valide via les stubs (même chemin que les
        // tests unitaires existants — garantit que l'XML passe le XSD).
        ...
        // Injection dans le process — sans subprocess exec().
        $writer = new \Atgp\FacturX\Writer();
